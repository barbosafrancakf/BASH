#! /bin/bash
RED="\033[31m";Yellow="\033[33m";Green="\e[32m";STD="\033[0;0;39m";Brown="\033[0;33m";servername="$(hostname)";operatingsystem="$(lsb_release -si)";operatingsystemrelease="$(lsb_release -sr)";bits="$(uname -m | sed "s/x86_//;s/i[3-6]86/32/")";ip=$(curl -s http://ifconfig.me);
...
read epassword
zip -r $date.zip $ldirectory
sshpass -p $epassword scp $date.zip $euser@$eaddress:$ldirectory
